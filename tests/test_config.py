...
def test_settings_from_env(monkeypatch):
    """从环境变量读取配置"""
    monkeypatch.setenv("FOUNDRY_AOAI_API_KEY", "k123")
    monkeypatch.setenv("FOUNDRY_AOAI_BASE_URL", "https://x/openai/v1/")
...
def test_settings_cu_optional(monkeypatch):
    """CU 配置可选，缺省为空字符串"""
    monkeypatch.setenv("FOUNDRY_AOAI_API_KEY", "k")
    monkeypatch.setenv("FOUNDRY_AOAI_BASE_URL", "https://x/openai/v1/")
    monkeypatch.delenv("FOUNDRY_CU_BASE_URL", raising=False)
    s = Settings()
